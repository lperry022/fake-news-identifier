  ...
  return (
    <div>
      ...
      <main className="container-page pb-24">
        ...
        {/* Search bar card */}
        <section className="mx-auto mt-8 max-w-2xl card p-4 relative">
        ...
        <section className="mx-auto mt-14 grid max-w-6xl grid-cols-1 gap-6 md:grid-cols-2">
          <div className="tile">
            ...
            <div className="mt-1 text-[11px] uppercase tracking-wide text-slate-400">articles assessed (demo)</div>
          </div>
          <div className="tile">
            <div className="text-4xl font-bold tabular-nums">15,098</div>
            <div className="mt-1 text-[11px] uppercase tracking-wide text-slate-400">explanations generated (demo)</div>
          </div>
        </section>
      </main>
    ...
    <header className="sticky top-0 z-10 border-b border-slate-800/60 bg-slate-950/70 backdrop-blur">
      <div className="container-page flex items-center justify-between py-3">
        <a href="#" className="inline-flex items-center gap-2">
          ...
          <span className="ml-1 rounded-md bg-fuchsia-500/20 px-1.5 py-0.5 text-xs text-fuchsia-300 ring-1 ring-fuchsia-500/30">beta</span>
        </a>
  ...
  return (
    <section className="pt-16 text-center">
      <h1 className="text-balance text-5xl md:text-7xl font-extrabold tracking-tight leading-tight">
        <span className="block">News credibility</span>
        <span className="block text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-fuchsia-400">
          checked in seconds
        </span>
      </h1>
      <p className="mx-auto mt-4 max-w-2xl text-lg text-slate-300">
        Paste a headline or URL to get a clear credibility score with concise explanations.
      </p>
    </section>
  );
}
...
function ResultPanel({ result }) {
  ...
  const chipClass =
    score >= 0.75 ? "pill-green" :
    score >= 0.4 ? "pill-amber" : "pill-rose";
    ...
    <section className="mx-auto mt-8 max-w-3xl">
      <div className="card p-6 relative">
        <div className="flex items-center justify-between">
          ...
          <span className={chipClass}>{label} confidence</span>
        </div>
        <div className="mt-4 h-2 w-full overflow-hidden rounded-lg bg-slate-800">
    ...
    <footer id="about" className="mt-16 border-t border-slate-800/60 py-12 text-sm text-slate-400">
      <div className="container-page grid gap-8 md:grid-cols-3">
        <div>
          ...
          <p className="mt-2">A student project that estimates credibility of news headlines and URLs.</p>
        </div>
        <div id="how">
      ...
      <p className="container-page mt-10 text-xs text-slate-500">
        © {new Date().getFullYear()} Team Fake News Identifier — Built with React & Tailwind.
      </p>
    </footer>
  );
}
...
function Logo() {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" className="text-cyan-400">
